...
param(
    [string]$Source = (Join-Path $PSScriptRoot 'Lumen_Optics_Spend_Case_Study.md'),
    [string]$Html   = (Join-Path $PSScriptRoot 'Lumen_Optics_Spend_Case_Study.html'),
    [string]$Pdf    = (Join-Path $PSScriptRoot 'Lumen_Optics_Spend_Case_Study.pdf')
)
...
$css = @'
:root{
  --ink:#1b2430; --muted:#5b6b7f; --rule:#dfe5ec; --navy:#1f3864;
  --band:#f5f7fa; --accent:#22508f; --red:#9c0006; --redbg:#ffc7ce;
...
<title>Lumen Optics Manufacturing &mdash; Photonics Spend Case Study</title>
<style>$css</style>
</head>
<body>
$($out.ToString())
